...
from keras.layers import LSTM, GRU, Dense, Input, Concatenate, Lambda, Conv1D, BatchNormalization
...
#X_test_holdout = X_test_holdout.reshape(X_test_holdout.shape[0], 1, -1)
...
forward = GRU(30, activation='elu', return_sequences=True)(inputs)
forward = GRU(50, activation='elu')(forward)
backward = GRU(30, activation='elu', return_sequences=True)(inputs)
backward = GRU(50, go_backwards=True, activation='elu')(backward)
...
pd.DataFrame({"Date": y_holdout_dates, "VAR": y_holdout_pred}).to_csv(
    os.path.join(SUBMISSIONS, "gru_v7.csv"), index=None)